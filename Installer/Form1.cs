	...
	public partial class Form1 : Form {
		...
		private void Download () {
			...
			using (WebClient wc = new WebClient ()) {
				...
				wc.DownloadFileAsync ( new Uri(downloadUrl), Path.Combine(tempFolder, "AutobuyerFifa16.zip"));
			}
		}

		private void Extract() {
			using (ZipFile zipFile = ZipFile.Read(Path.Combine(tempFolder, zipName))) {
				zipFile.ExtractAll(installFolder, ExtractExistingFileAction.OverwriteSilently);
			}
			EndOperation();
		}
